...
class OperationType(Enum):
    ...
    def get_job_status(self, job_id: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "status": job_status_map.get(ProcessingStatus(session["status"]), "unknown"),
            ...
            "elapsed_time": (
                (datetime.utcnow() - datetime.fromisoformat(session["created_at"])).total_seconds()
                if session.get("created_at") else 0
            ),
    ...
    def _estimate_remaining_time(self, session: Dict[str, Any]) -> Optional[float]:
        ...
        elapsed_seconds = (datetime.utcnow() - datetime.fromisoformat(session["created_at"])).total_seconds()
        rate = metrics["processed_items"] / elapsed_seconds if elapsed_seconds > 0 else 0
    ...
    def get_active_jobs(self, user_id: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        with self.session_lock:
            for session in self.active_sessions.values():
                if session.status in active_statuses and (user_id is None or session.user_id == user_id):
                    job_status = self.get_job_status(session.session_id)
                    if job_status:
                        jobs.append(job_status)
        return jobs
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        with self.session_lock:
            total_jobs = len(self.active_sessions)
            active_jobs = sum(
            ...
            return {
                ...
                "cancelled_jobs": sum(
                    1 for session in self.active_sessions.values()
                    if session.status == ProcessingStatus.CANCELLED
                ),
...
progress_tracker = ProgressTracker() parsing operations."""
